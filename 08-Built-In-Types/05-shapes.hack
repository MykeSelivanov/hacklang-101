...
// Shapes have the same runtime representation as darray, although this is considered an implementation detail. 
// This representation means that shape order is observable.
$s1 = shape('name' => 'db-01', 'age' => 365);
...
// This is useful because it promotes code re-use and when the same type is being used, and provides a descriptive name for the type.
...
// Any shape value that has all of the required fields (and no undefined fields - unless the shape permits them) is considered 
// a value of type Server; the type is not specified when creating the value.
function takes_server(Server $s): void {
  return;
}
...
// Two shapes have the same type if they have the same fields and types. This makes shapes convenient to create, but can cause surprises. 
